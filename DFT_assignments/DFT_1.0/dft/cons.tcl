...
set CLK_PER 100
set SCAN_PER 200
set SCAN_CLK_NAME SCAN_CLOCK
set SCAN_SETUP_SKEW 0.2
set SCAN_HOLD_SKEW 0.1
set CLK_SETUP_SKEW 0.2
set CLK_HOLD_SKEW 0.1
set CLK_LAT 0
set CLK_RISE 0.05
set CLK_FALL 0.05
set SCAN_TRANSITION 0.05
create_clock -name $CLK_NAME -period $CLK_PER -waveform "0 [expr $CLK_PER/2]" [get_ports CLK]
create_clock -name $SCAN_CLK_NAME -period $SCAN_PER -waveform "0 [expr $SCAN_PER/2]" [get_ports scan_clk]
...
set_clock_uncertainty -setup $SCAN_SETUP_SKEW [get_clocks $SCAN_CLK_NAME]
set_clock_uncertainty -hold  $SCAN_HOLD_SKEW  [get_clocks $SCAN_CLK_NAME]
set_clock_transition  $SCAN_TRANSITION [get_clocks $SCAN_CLK_NAME]
...
set_dont_touch_network [list $CLK_NAME $SCAN_CLK_NAME]
...
####################################################################################
           ...
           #########################################################
####################################################################################
set_clock_groups -asynchronous -group $CLK_NAME -group $SCAN_CLK_NAME
...
set scan_in_delay [expr 0.2*$SCAN_PER]
set scan_out_delay [expr 0.2*$SCAN_PER]
...
set_input_delay $scan_in_delay -clock $SCAN_CLK_NAME [get_port SI]
set_input_delay $scan_in_delay -clock $SCAN_CLK_NAME [get_port SE]
set_input_delay $scan_in_delay -clock $SCAN_CLK_NAME [get_port test_mode]
...
set_output_delay $scan_out_delay -clock $SCAN_CLK_NAME [get_port SO]
...
#scan ports
set_driving_cell -library scmetro_tsmc_cl013g_rvt_ss_1p08v_125c -lib_cell BUFX2M [get_port SI]
set_driving_cell -library scmetro_tsmc_cl013g_rvt_ss_1p08v_125c -lib_cell BUFX2M [get_port SE]
set_driving_cell -library scmetro_tsmc_cl013g_rvt_ss_1p08v_125c -lib_cell BUFX2M [get_port test_mode]
